...
Запускает приложение (требует скомпилированный MCP сервер).
...
1. **MCP Client** (`src/mcp-clients/index.ts`)
   ...
   - Загружает доступные ресурсы и prompts при подключении к серверу
   - Поддерживает вызов registered prompts через команду `/prompt`
...
3. **MCP Servers** (`src/mcp-servers/`)
   ...
   - **Prompts**: регистрирует предустановленные prompts для специализированных задач (например, `summarize-doc-content`)
...
3. **Запуск приложения**
   ...
   - Доступные команды:
     ...
     - `/prompt <name> <arg1=value1> [arg2=value2]` - использовать зарегистрированный prompt
