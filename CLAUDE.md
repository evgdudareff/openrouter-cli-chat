...
1. **MCP Client** (`src/mcp-clients/mcp-client.ts`)
   ...
   - Обрабатывает извлечение определений инструментов из серверов MCP
...
3. **MCP Servers** (`src/mcp-servers/`)
   - Независимые процессы, которые предоставляют инструменты через протокол MCP
   - В настоящее время включает `file-mcp-server` для чтения локальных документов
   - Использует stdio транспорт для связи с клиентом
   - Пример: инструмент `get_user_docs` читает файлы из `src/mcp-servers/file-mcp-server/docs/`
